...
def _format_question(raw_question: str, is_math: bool) -> str:
    ...
    if is_math:
        if '####' not in question and MATH_INSTRUCTION:
            question = f"{question}\n\n{MATH_INSTRUCTION}"
        return question
...
if __name__ == '__main__':
    ...
    for data_source in data_sources:
        ...
        # random sample
        sample_size = min(len(test_dataset), 500)
    ...
    if last_canonical_source is None:
        file_source = 'unknown'
    else:
        file_source = last_canonical_source.replace('/', '_')
